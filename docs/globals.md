...
* **Description:**
  ...
  For a long time, the `"EMPTY"` keyword convention (see [`DBPutMultimesh()`](./parallel.md#dbputmultimesh)) was sufficient for dealing with cases where callers needed to create multiple, related multi-block objects with missing blocks.
