...
offset=0
for i in range(0,newzl["nshapes"]):
    len = newzl["shapecnt"][i]*newzl["shapesize"][i]
    exconn = [nid+1 for nid in newzl["nodelist"][offset:offset+len]]
...
for v in fdata["zonals"]:
    ...
    for i in range(0,newzl["nshapes"]):
        len = newzl["shapecnt"][i]
        vals = [zid for zid in fdata["zonals"][v][offset:offset+len]]
