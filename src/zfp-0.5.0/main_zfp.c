...
C_STRUCTSPACE_STATIC zfp_field*
zfp_field_alloc()
{
...
C_STRUCTSPACE_STATIC zfp_stream*
...
C_STRUCTSPACE_STATIC bitstream*
...
{
  return zfpbs.stream_size(zfp->stream);
}
...
zfp_stream_maximum_size(const zfp_stream* zfp, const zfp_field* field)
{
  ...
  return ((ZFP_HEADER_BITS + blocks * maxbits + zfpbs.stream_word_bits - 1) & ~(zfpbs.stream_word_bits - 1)) / CHAR_BIT;
}
...
/* return zfp computed rate as a rational number in numer and denom args */
C_STRUCTSPACE_STATIC uint
zfp_stream_set_rate_rational(zfp_stream* zfp, double rate, zfp_type type, uint dims, int wra,
    uint *numer, uint *denom)
{
  double _rate = zfp_stream_set_rate(zfp, rate, type, dims, wra);
  uint n = 1u << (2 * dims);
  *denom = n;
  *numer = (uint) (_rate * n);
  return *numer;
...
/* return zfp computed tolerance as an unsigned bit count */
C_STRUCTSPACE_STATIC uint
zfp_stream_set_accuracy_log2(zfp_stream* zfp, double tolerance, zfp_type type)
{
    double _tolerance = zfp_stream_set_accuracy(zfp, tolerance, type);
    return (uint) log2(_tolerance);
}
...
{
  zfpbs.stream_flush(zfp->stream);
}

C_STRUCTSPACE_STATIC void
zfp_stream_align(zfp_stream* zfp)
{
  zfpbs.stream_align(zfp->stream);
}

C_STRUCTSPACE_STATIC void
zfp_stream_rewind(zfp_stream* zfp)
{
  zfpbs.stream_rewind(zfp->stream);
}
...
zfp_write_header(zfp_stream* zfp, const zfp_field* field, uint mask)
{
  ...
  if (mask & ZFP_HEADER_MAGIC) {
    zfpbs.stream_write_bits(zfp->stream, 'z', 8);
    zfpbs.stream_write_bits(zfp->stream, 'f', 8);
    zfpbs.stream_write_bits(zfp->stream, 'p', 8);
    zfpbs.stream_write_bits(zfp->stream, ZFP_VERSION >> 4, 8);
    bits += ZFP_MAGIC_BITS;
  }
  /* 52-bit field metadata */
  if (mask & ZFP_HEADER_FIELD) {
    uint64 meta = zfp_field_metadata(field);
    zfpbs.stream_write_bits(zfp->stream, meta, ZFP_META_BITS);
    bits += ZFP_META_BITS;
  }
  ...
  if (mask & ZFP_HEADER_PARAMS) {
    ...
    zfpbs.stream_write_bits(zfp->stream, mode, size);
    bits += size;
  }
  return bits;
}
...
zfp_read_header(zfp_stream* zfp, zfp_field* field, uint mask)
{
  size_t bits = 0;
  if (mask & ZFP_HEADER_MAGIC) {
    if (zfpbs.stream_read_bits(zfp->stream, 8) != 'z' ||
        zfpbs.stream_read_bits(zfp->stream, 8) != 'f' ||
        zfpbs.stream_read_bits(zfp->stream, 8) != 'p' ||
        zfpbs.stream_read_bits(zfp->stream, 8) != (ZFP_VERSION >> 4))
      return 0;
    bits += ZFP_MAGIC_BITS;
  }
  if (mask & ZFP_HEADER_FIELD) {
    uint64 meta = zfpbs.stream_read_bits(zfp->stream, ZFP_META_BITS);
    if (!zfp_field_set_metadata(field, meta))
      return 0;
    bits += ZFP_META_BITS;
  }
  if (mask & ZFP_HEADER_PARAMS) {
    uint64 mode = zfpbs.stream_read_bits(zfp->stream, ZFP_MODE_SHORT_BITS);
    bits += ZFP_MODE_SHORT_BITS;
    if (mode > ZFP_MODE_SHORT_MAX) {
      uint size = ZFP_MODE_LONG_BITS - ZFP_MODE_SHORT_BITS;
      mode += zfpbs.stream_read_bits(zfp->stream, size) << ZFP_MODE_SHORT_BITS;
      bits += size;
    }
