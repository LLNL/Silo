...
_t2(decompress, Scalar, 1)(zfp_stream* stream, zfp_field* field)
{
  ...
  for (x = 0; x < mx; x += 4, data += 4)
    _t2(zfpns.zfp_decode_block, Scalar, 1)(stream, data);
  if (x < nx)
    _t2(zfpns.zfp_decode_partial_block_strided, Scalar, 1)(stream, data, nx - x, 1);
}
...
_t2(decompress_strided, Scalar, 1)(zfp_stream* stream, zfp_field* field)
{
  ...
  for (x = 0; x < nx; x += 4) {
    ...
    if (nx - x < 4)
      _t2(zfpns.zfp_decode_partial_block_strided, Scalar, 1)(stream, p, nx - x, sx);
    else
      _t2(zfpns.zfp_decode_block_strided, Scalar, 1)(stream, p, sx);
  }
}
...
_t2(decompress_strided, Scalar, 2)(zfp_stream* stream, zfp_field* field)
{
  ...
  for (y = 0; y < ny; y += 4)
    for (x = 0; x < nx; x += 4) {
      ...
      if (nx - x < 4 || ny - y < 4)
        _t2(zfpns.zfp_decode_partial_block_strided, Scalar, 2)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), sx, sy);
      else
        _t2(zfpns.zfp_decode_block_strided, Scalar, 2)(stream, p, sx, sy);
    }
}
    ...
    for (y = 0; y < ny; y += 4)
      for (x = 0; x < nx; x += 4) {
        ...
        if (nx - x < 4 || ny - y < 4 || nz - z < 4)
          _t2(zfpns.zfp_decode_partial_block_strided, Scalar, 3)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), MIN(nz - z, 4u), sx, sy, sz);
        else
          _t2(zfpns.zfp_decode_block_strided, Scalar, 3)(stream, p, sx, sy, sz);
      }
}
      ...
      for (y = 0; y < ny; y += 4)
        for (x = 0; x < nx; x += 4) {
          ...
          if (nx - x < 4 || ny - y < 4 || nz - z < 4 || nw - w < 4)
            _t2(zfpns.zfp_decode_partial_block_strided, Scalar, 4)(stream, p, MIN(nx - x, 4u), MIN(ny - y, 4u), MIN(nz - z, 4u), MIN(nw - w, 4u), sx, sy, sz, sw);
          else
            _t2(zfpns.zfp_decode_block_strided, Scalar, 4)(stream, p, sx, sy, sz, sw);
        }
}
